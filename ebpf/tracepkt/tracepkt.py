...
def event_printer(cpu, data, size):
    ...
    # Print event
    print("%-30s [%12s] [%12s] %-16s %-7s %-34s%-30s" % (event.funcname.decode('utf-8'), event.ts, event.netns, event.ifname.decode('utf-8'), direction, flow, iptables))
...
if __name__ == "__main__":
    ...
    print("%-30s %14s %14s %16s %7s %-34s %-30s" % ("TRACEPOINT", "TIMESTAMP", 'NETWORK NS', 'INTERFACE', 'TYPE', 'ADDRESSES', 'IPTABLES'))
